...
export const walletController = {
    ...
    refundWallet: async (req, res) => {
        try {
            ...
            const result = await walletService.refundWallet(Number(userId), originalTrancheId, Number(amountToRefund), reason || "Remboursement client");
    ...
    handleKkiapayWebhook: async (req, res) => {
        try {
            ...
            const tranche = await walletService.creditWallet(Number(metadata.userId), Number(amount), "KKIAPAY", transactionId || `KKIA-${Date.now()}`, metadata.countryCode || null);
    ...
    simulateTestRecharge: async (req, res) => {
        try {
            ...
            const tranche = await walletService.creditWallet(userId, Number(amount), "SYSTEM", `SANDBOX-${method}-${Date.now()}`, "BJ");
