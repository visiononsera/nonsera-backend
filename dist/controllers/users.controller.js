...
export const usersController = {
    ...
    updateOnboardingProfile: async (req, res) => {
        try {
            ...
            const { fullname, email, birthday, gender, pin, code, religion, passions, height, biography, vision, nextStep, } = req.body;
            ...
            if (updatedUser.onboardingStep === "CALL_VALIDATION") {
                ...
                // @ts-ignore 
                const videoSession = await videoValidationService.initializeSession(userId, true);
                return res.status(200).json({
                    success: true,
                    code: "ONBOARDING_COMPLETED_AWAITING_CALL",
                    message: "Informations enregistrées. En attente de la vérification par un agent.",
                    data: {
    ...
    deactivateAccount: async (req, res) => {
        try {
            ...
            return res.status(200).json({
                success: true,
                message: "Votre compte a été suspendu temporairement avec succès. Vous avez été déconnecté.",
            });
        }
        catch (error) {
            return res.status(500).json({ success: false, message: error.message });
    ...
    deleteAccount: async (req, res) => {
        try {
            ...
            return res.status(200).json({
                ...
                message: "Conformité Google : Votre compte et l'ensemble de vos données personnelles ont été supprimés définitivement.",
            });
        }
        catch (error) {
            return res.status(500).json({ success: false, message: error.message });
    ...
    assignPermissionsToUser: async (req, res) => {
        try {
            ...
            if (!targetUserId || !Array.isArray(permissionIds)) {
                return res.status(400).json({
                    success: false,
                    message: "L'identifiant de la cible et un tableau d'IDs de permissions sont requis.",
                });
            }
            ...
            if (targetUser.role === "IT" && requesterRole === "ADMIN") {
                return res.status(403).json({
                    success: false,
                    message: "Rupture de privilèges : Un Administrateur ne peut modifier les privilèges d'un profil IT.",
                });
            }
            await prisma.user.update({
                where: { id: parseInt(targetUserId) },
     ...
     */
    createSystemPermission: async (req, res) => {
        try {
            ...
            return res.status(201).json({
                success: true,
                message: "Nouvelle règle de privilège injectée dans le dictionnaire système.",
                data: newPermission,
            });
        }
        catch (error) {
            return res.status(400).json({
                success: false,
                message: "Impossible de créer la permission. Le code est peut-être déjà utilisé.",
            });
        }
    },
    // ==========================================
    ...
    createStaffAccount: async (req, res) => {
        try {
            ...
            if (!phoneNumber || !fullname || !role || !passCode) {
                return res.status(400).json({
                    success: false,
                    message: "Tous les champs (phoneNumber, fullname, role, passCode) sont obligatoires.",
                });
            }
            if (role === "IT" && requesterRole === "ADMIN") {
                return res.status(403).json({
                    success: false,
                    message: "Création refusée. Seul un administrateur système IT peut initialiser un autre profil IT.",
                });
            }
     ...
     */
    reviewPendingAccount: async (req, res) => {
        try {
            const { targetUserId, action, reason } = req.body;
            if (!targetUserId || !action) {
                return res.status(400).json({
                    success: false,
                    message: "L'identifiant de la cible (targetUserId) et l'action ('APPROVE' | 'REJECT' | 'BAN') sont requis.",
                });
            }
     ...
     */
    getPendingOnboardings: async (req, res) => {
        try {
