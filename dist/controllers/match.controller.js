...
export class MatchController {
    ...
    static async sendDirectGift(req, res, next) {
        try {
            ...
            const result = await MatchService.sendGiftProposal(Number(senderId), Number(receiverId), formattedGiftId, formattedAnnonceId);
    ...
    static async acceptDirectGift(req, res, next) {
        try {
            ...
            const result = await MatchService.acceptDirectGift(Number(receiverId), Number(senderId), formattedGiftId, formattedAnnonceId, matchType || "NORMAL");
            return res.status(200).json({
                success: true,
