...
export const reservationsService = {
    ...
    create: async (data) => {
        ...
        return await prisma.$transaction(async (tx) => {
            ...
            await walletService.debitWallet(data.userId, totalPrice, `Réservation ${reference} - ${annonce.name}`, tx);
    ...
    resolveDispute: async (id, decision, adminNotes) => {
        ...
        if (decision === "REFUND_CLIENT") {
            ...
            await walletService.refundWallet(reservation.userId, historicalTrancheId, Number(reservation.totalPrice), `Remboursement arbitrage litige : ${adminNotes}`);
            return await prisma.reservation.update({
                where: { id },
                data: {
    ...
    cancel: async (id, actor, reason) => {
        ...
        if (shouldRefund && reservation.totalPrice) {
            await walletService.refundWallet(reservation.userId, historicalTrancheId, Number(reservation.totalPrice), `Annulation réservation ${reservation.reference}`);
        }
