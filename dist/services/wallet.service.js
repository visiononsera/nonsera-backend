...
export const walletService = {
     ...
     */
    creditWallet: async (userId, amount, provider, reference, customCountryCode = null) => {
     ...
     */
    debitWallet: async (userId, totalAmountToDebit, description, txClient = null, isTransferLumiere = false) => {
        const executeLogic = async (tx) => {
            ...
            for (const tranche of activeTranches) {
                ...
                let newStatut = newPrincipalRestant === 0 ? 'EPUISE' : 'ACTIVE';
     ...
     */
    transferLumiere: async (senderId, receiverId, amountToTransfer) => {
        return await prisma.$transaction(async (tx) => {
    ...
    refundWallet: async (userId, originalTrancheId, amountToRefund, reason) => {
        return await prisma.$transaction(async (tx) => {
            ...
            if (!tranche)
                throw new Error("Tranche d'origine introuvable.");
            const pInitial = Number(tranche.principalInitial);
            const bTotal = Number(tranche.bonusTotal);
    ...
    getWalletSummary: async (userId, txClient = null) => {
        ...
        if (firstTranche) {
            ...
            if (config)
                currentCurrencySymbol = config.symbol;
        }
        return {
