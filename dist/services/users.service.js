...
export const usersService = {
    getMany: async (filters = {}, pagination = { limit: 8, page: 0 }) => {
        const limit = parseInt(pagination.limit, 10) || 8;
        const page = parseInt(pagination.page, 10) || 0;
        const { phoneNumber, role, isCertified, isOnline, pays, villes, genre, excludeUserId, search, } = filters;
        const whereClause = { isBanned: false };
        if (role !== undefined && role !== null && role !== "")
            whereClause.role = role;
        if (phoneNumber && phoneNumber.trim() !== "")
            whereClause.phoneNumber = phoneNumber.trim();
        if (excludeUserId) {
            const exId = typeof excludeUserId === "string"
                ? parseInt(excludeUserId, 10)
                : excludeUserId;
            if (!isNaN(exId))
                whereClause.NOT = { id: exId };
        }
     ...
     */
    generateUniqueUsername: async (fullname) => {
     ...
     */
    updateOnboardingData: async (userId, inputData) => {
        ...
        if (inputData.fullname) {
            ...
            updatePayload.username = await usersService.generateUniqueUsername(inputData.fullname);
        }
        ...
        if (inputData.profilePhoto) {
            ...
            if (user.isCompleted && user.lastPhotoUpdated) {
                ...
                const nextAllowedUpdate = new Date(lastUpdate.getTime() + 2 * 24 * 60 * 60 * 1000);
                if (new Date() < nextAllowedUpdate) {
                    throw new Error("Sécurité : Vous ne pouvez modifier votre photo de profil que tous les 2 jours.");
                }
            }
        ...
        if (inputData.vision !== undefined)
            updatePayload.vision = inputData.vision;
        ...
        if (inputData.email)
            updatePayload.email = inputData.email;
        if (inputData.birthday)
            updatePayload.birthday = new Date(inputData.birthday);
        if (inputData.gender)
            updatePayload.gender = inputData.gender;
        ...
        if (inputData.nextStep)
            updatePayload.onboardingStep = inputData.nextStep;
     ...
     */
    deleteAccountCompletely: async (userId) => {
