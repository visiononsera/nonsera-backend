...
export const annoncesService = {
    ...
    create: async (data, file) => {
        ...
        if (!company) {
            throw new Error("Impossible de créer l'annonce : l'entreprise partenaire spécifiée n'existe pas.");
        }
        ...
        if (file) {
            ...
            if (annonce.image && annonce.image !== imageUrl) {
                ...
                storageService
                    ...
                    .catch((err) => console.error(`Impossible de supprimer l'ancienne image de l'annonce ${id} :`, err));
            }
        }
        return await prisma.annonce.update({
            ...
            data: {
                ...
                category: updateData.category !== undefined
                    ...
                    : undefined,
                ambiance: updateData.ambiance !== undefined
                    ? (updateData.ambiance ?? null)
                    : undefined,
                vehicleType: updateData.vehicleType !== undefined
                    ? (updateData.vehicleType ?? null)
                    : undefined,
                nbPlaces: updateData.nbPlaces !== undefined
                    ? (updateData.nbPlaces ?? null)
                    : undefined,
                activityType: updateData.activityType !== undefined
     ...
     */
    getMany: async (filters = {}, pagination = { limit: 10, page: 0 }) => {
        const limit = pagination.limit ?? 10;
        const page = pagination.page ?? 0;
        const { companyId, category, vehicleType, activityType, isAvailable, isVerified, search, } = filters;
