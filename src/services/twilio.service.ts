...
if (!TWILIO_ACCOUNT_SID || !TWILIO_AUTH_TOKEN) {
  throw new Error(
    "Les identifiants Twilio sont manquants dans les variables d'environnement.",
  );
}
...
export const twilioService = {
  ...
  sendOtp: async (phoneNumber: string) => {
    if (!phoneNumber) {
      throw new Error(
        "Le numéro de téléphone est requis pour l'envoi de l'OTP.",
      );
    }
    ...
    if (IS_DEV) {
      console.log(
        `[TWILIO DEV MODE] OTP simulé envoyé avec succès au : ${phoneNumber}`,
      );
    ...
    try {
      ...
      throw new Error(
        `Échec de l'envoi du SMS de vérification : ${error.message}`,
      );
    }
  },
  ...
  verifyOtp: async (phoneNumber: string, code: string) => {
    if (!phoneNumber || !code) {
      throw new Error(
        "Le numéro de téléphone et le code de vérification sont requis.",
      );
    }
    ...
    if (IS_DEV) {
      ...
      if (code === "001089") {
        console.log(
          `[TWILIO DEV MODE] Code de test validé avec succès pour : ${phoneNumber}`,
        );
        return { success: true, status: "approved" };
      }
      throw new Error(
        "Le code de vérification soumis est invalide (Mode Test).",
      );
    }
    ...
    try {
      ...
      throw new Error(
        error.message || "Impossible de valider le code de vérification.",
      );
    }
  },
   ...
   */
  sendCustomSms: async (toPhoneNumber: string, messageBody: string) => {
    if (!toPhoneNumber || !messageBody) {
      throw new Error(
        "Le numéro de téléphone destinataire et le corps du message sont requis.",
      );
    }
    ...
    try {
      ...
      throw new Error(
        `Échec de l'envoi du SMS personnalisé : ${error.message}`,
      );
    }
  },
};
