...
export class MatchController {
  ...
  static async sendDirectGift(req: Request, res: Response) {
    try {
      ...
      if (!senderId || !receiverId || !giftId) {
        return res.status(400).json({ error: 'Paramètres manquants (senderId, receiverId, giftId).' });
      }
  ...
  static async acceptDirectGift(req: Request, res: Response) {
    try {
      ...
      if (!receiverId || !senderId || !giftId) {
        return res.status(400).json({ error: 'Paramètres manquants (receiverId, senderId, giftId).' });
      }
      ...
      return res.status(200).json({
        success: true,
        message: 'Cadeau accepté avec succès. Le couple est créé et le salon privé est ouvert.',
        data: result
      });
    } catch (error: any) {
      return res.status(500).json({ error: error.message });
  ...
  static async breakMatch(req: Request, res: Response) {
    try {
      ...
      if (!userId || !partnerId) {
        return res.status(400).json({ error: 'Paramètres manquants (userId, partnerId).' });
      }

      const result = await MatchService.breakMatch(Number(userId), Number(partnerId));
      return res.status(200).json(result);
    } catch (error: any) {
      return res.status(500).json({ error: error.message });
    }
