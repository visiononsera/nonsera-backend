...
export const walletController = {
   ...
   */
  getSummary: async (req: AuthenticatedRequest, res: Response): Promise<Response> => {
    try {
   ...
   */
  debit: async (req: AuthenticatedRequest, res: Response): Promise<Response> => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ success: false, message: "Utilisateur non authentifié." });
   ...
   */
  transferLumiere: async (req: AuthenticatedRequest, res: Response): Promise<Response> => {
    try {
   ...
   */
  refundWallet: async (req: AuthenticatedRequest, res: Response): Promise<Response> => {
    try {
   ...
   */
  lockBonus: async (req: AuthenticatedRequest, res: Response): Promise<Response> => {
    try {
      const userId = req.user?.id; // Ou passé dans le body si c'est une action admin
      ...
      const success = await walletService.lockBonus(targetUserId, Number(amountToLock), reason || "Blocage de sécurité");
   ...
   */
  unlockBonus: async (req: AuthenticatedRequest, res: Response): Promise<Response> => {
    try {
      const userId = req.user?.id;
      const targetUserId = req.body.userId ? Number(req.body.userId) : userId;
      ...
      const success = await walletService.unlockBonus(targetUserId, Number(amountToUnlock), reason || "Libération de bonus");
   ...
   */
  expireOldBonus: async (req: Request, res: Response): Promise<Response> => {
    try {
   ...
   */
  handleKkiapayWebhook: async (req: Request, res: Response): Promise<Response> => {
    try {
   ...
   */
  simulateTestRecharge: async (req: AuthenticatedRequest, res: Response): Promise<Response> => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ success: false, message: "Utilisateur non authentifié." });
