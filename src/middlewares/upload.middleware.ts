 ...
 */
const getStorageEngine = (mediaType: MediaType): multer.StorageEngine => {
  const isRemote =
    STORAGE_PROVIDER?.toUpperCase() === "S3" ||
  ...
  if (isRemote) {
    ...
    return multer.diskStorage({
      ...
      filename: (req: any, file, cb) => {
        ...
        cb(
          null,
          `${ownerId}-${uniqueSuffix}${path.extname(file.originalname)}`
        );
      },
    });
  }
...
// ======================================================
// FABRIQUE DE MIDDLEWARES (MIDDLEWARE FACTORY)
// ======================================================
